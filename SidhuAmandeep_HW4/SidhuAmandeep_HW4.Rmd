...
$$ = -argmin_{\gamma_{jm}} \sum_{x_i\in R_{jm}} y_ilog(f_{m-1}(x_i) + \gamma_{jm}) + (1-y_i)log(1-f_{m-1}(x_i) - \gamma_{jm})$$

Take the derivative with respect to $\gamma_{jm}$:
...
Note, here because we are dealing with the binomial deviance (or equivalently the log-likelihood loss function), $y_i$ is the observed probability and $f_{m-1}(x_i)$ is the predicted probability. So, $\gamma_{jm}$ is simply the difference between the two.
...
$$ L = y_ilog(f_{m-1}(x_i) + \gamma_{jm}) + (1-y_i)log(1 - f_{m-1}(x_i) - \gamma_{jm}) $$
...
$$ \gamma = \frac{L'(y, f_{m-1}(x_i))}{L''(y_i, f_{m-1}(x_i))}$$
$$ \gamma = \frac{\frac{y_i - f_{m-1}(x_i)}{(f_{m-1}(x_i))(1-f_{m-1}(x_i)}}{\frac{f_{m-1}^2(x_i) - 2y_i f_{m-1}(x_i) + y_i}{(f_{m-1}(x_i)-1)^2f_{m-1}^2(x_i)}} $$
